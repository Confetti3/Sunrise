...
bool SettingsWriter::put_u8(std::uint8_t value) noexcept {
...
bool SettingsWriter::put_bool(bool value) noexcept {
    return put_u8(value ? 1U : 0U);
}

bool SettingsWriter::put_u16(std::uint16_t value) noexcept {
    return put_u8(static_cast<std::uint8_t>(value & 0xFFU))
           && put_u8(static_cast<std::uint8_t>((value >> 8U) & 0xFFU));
}

bool SettingsWriter::put_u32(std::uint32_t value) noexcept {
    return put_u8(static_cast<std::uint8_t>(value & 0xFFU))
           && put_u8(static_cast<std::uint8_t>((value >> 8U) & 0xFFU))
...
bool SettingsWriter::put_i8(std::int8_t value) noexcept {
    return put_u8(std::bit_cast<std::uint8_t>(value));
}

bool SettingsWriter::put_i32(std::int32_t value) noexcept {
    return put_u32(std::bit_cast<std::uint32_t>(value));
}

bool SettingsWriter::put_float(float value) noexcept {
    return put_u32(std::bit_cast<std::uint32_t>(value));
}

bool SettingsWriter::put_optional_u16(const std::optional<std::uint16_t>& value) noexcept {
    return put_bool(value.has_value()) && put_u16(value.value_or(0));
}

std::span<const std::byte> SettingsWriter::bytes() const noexcept {
    return {bytes_.data(), size_};
}

SettingsReader::SettingsReader(std::span<const std::byte> bytes) noexcept : bytes_(bytes) {}

bool SettingsReader::get_u8(std::uint8_t& output) noexcept {
...
bool SettingsReader::get_bool(bool& output) noexcept {
    std::uint8_t value = 0;
...
bool SettingsReader::get_u16(std::uint16_t& output) noexcept {
    std::uint8_t low = 0;
    std::uint8_t high = 0;
...
bool SettingsReader::get_u32(std::uint32_t& output) noexcept {
    std::uint8_t byte0 = 0;
    std::uint8_t byte1 = 0;
    std::uint8_t byte2 = 0;
    std::uint8_t byte3 = 0;
...
bool SettingsReader::get_i8(std::int8_t& output) noexcept {
    std::uint8_t value = 0;
    if (!get_u8(value)) {
        return false;
    }
...
bool SettingsReader::get_i32(std::int32_t& output) noexcept {
...
bool SettingsReader::get_float(float& output) noexcept {
    std::uint32_t value = 0;
    if (!get_u32(value)) {
        return false;
    }
...
bool SettingsReader::get_optional_u16(std::optional<std::uint16_t>& output) noexcept {
    bool present = false;
...
bool SettingsReader::complete() const noexcept {
    return offset_ == bytes_.size();
}

bool encode_settings(const account::settings::AccountSettings& value,
...
SettingsDecodeResult decode_settings(std::span<const std::byte> bytes,
