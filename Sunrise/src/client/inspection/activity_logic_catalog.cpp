...
bool validate(const Catalog& catalog, std::string& error) {
    ...
    if (catalog.activities.size() > kMaximumActivities
        || catalog.entities.size() > kMaximumEntities || catalog.edges.size() > kMaximumEdges) {
    ...
    for (const Entity& entity : catalog.entities) {
        if (entity.definitionTag == 0 || entity.classPrimary == 0 || entity.classSecondary == 0
            ...
            || !confidence_valid(entity.confidence) || entity.name.empty() || entity.label.empty()) {
    ...
    try {
        ...
        for (std::uint32_t index = 0; index < entitySection.count; ++index) {
            ...
            if (!reader.read_u32(offset, entity.definitionTag)
                ...
                || !reader.read_u8(offset + 12U, role)
                || !reader.read_u8(offset + 13U, confidence)
        ...
        for (std::uint32_t index = 0; index < activitySection.count; ++index) {
            ...
            for (std::uint32_t reference = 0; reference < referenceCount; ++reference) {
                ...
                if (!reader.read_u32(refOffset, entityIndex) || entityIndex >= catalog.entities.size()) {
...
const Activity* find_activity(const Catalog& catalog, std::uint32_t scenarioTag) noexcept {
    const auto iterator = std::lower_bound(
        ...
        scenarioTag,
        [](const Activity& activity, std::uint32_t value) {
            return activity.scenarioTag < value;
        });
    return iterator != catalog.activities.end() && iterator->scenarioTag == scenarioTag
               ? &*iterator
               : nullptr;
}
...
const Entity* find_entity(const Catalog& catalog, std::uint32_t definitionTag) noexcept {
    const auto iterator = std::lower_bound(
        ...
        definitionTag,
        [](const Entity& entity, std::uint32_t value) {
            return entity.definitionTag < value;
        });
...
const char* role_name(Role role) noexcept {
    switch (role) {
    case Role::actionSequence: return "Action sequence";
    case Role::actionTarget: return "Action target";
    case Role::competitiveRule: return "Competitive rule";
    case Role::conditionMonitor: return "Condition monitor";
    case Role::device: return "Device";
    case Role::object: return "Object / interactable";
    case Role::objective: return "Objective";
    case Role::spatialRule: return "Spatial rule / area";
    case Role::spawnDefinition: return "Squad spawn rule";
    case Role::squadDefinition: return "Squad / group";
    case Role::triggerSource: return "Trigger / volume";
