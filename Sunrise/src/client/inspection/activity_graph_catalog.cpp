    ...
    [[nodiscard]] bool read_u32(std::size_t offset, std::uint32_t& value) const noexcept {
        ...
        value = static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(bytes[offset]))
                | (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(bytes[offset + 1]))
                   << 8U)
                | (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(bytes[offset + 2]))
                   << 16U)
    ...
    while (index < bytes.size()) {
        ...
        for (std::size_t continuation = 1; continuation < length; ++continuation) {
            const std::uint8_t value =
                std::to_integer<std::uint8_t>(bytes[index + continuation]);
...
bool validate(const Catalog& catalog, std::string& error) {
    ...
    for (const Graph& graph : catalog.graphs) {
        ...
        for (const GraphNode& node : graph.nodes) {
            if (node.graphHash != graph.hash || node.nodeHash == 0
                || !std::isfinite(node.authoredX) || !std::isfinite(node.authoredY)) {
                return fail(error, "graph node identity or authored position is invalid");
            }
            const std::uint64_t key = (static_cast<std::uint64_t>(node.graphHash) << 32U)
                                      | node.nodeHash;
...
bool load(std::span<const std::byte> bytes, Catalog& catalog, std::string& error) {
    try {
        ...
        if (!safe_range(bytes.size(), stringOffset, stringSize, 1, kMaximumFileSize)
            || !safe_range(bytes.size(), sections[kActivitySection].offset,
                           sections[kActivitySection].count,
                           sections[kActivitySection].stride,
                           kMaximumActivities)
            || !safe_range(bytes.size(), sections[kGraphSection].offset,
                           sections[kGraphSection].count,
                           sections[kGraphSection].stride,
                           kMaximumGraphs)
            || !safe_range(bytes.size(), sections[kNodeSection].offset,
                           sections[kNodeSection].count,
                           sections[kNodeSection].stride,
                           kMaximumNodes)
            || !safe_range(bytes.size(), sections[kActivityRefSection].offset,
                           sections[kActivityRefSection].count,
                           sections[kActivityRefSection].stride,
                           kMaximumReferences)
            || !safe_range(bytes.size(), sections[kLinkedRefSection].offset,
                           sections[kLinkedRefSection].count,
                           sections[kLinkedRefSection].stride,
                           kMaximumReferences)
            || !safe_range(bytes.size(), sections[kLocationSection].offset,
        ...
        const auto add_range = [&](std::uint32_t offset,
                                   std::uint32_t count,
                                   std::uint32_t stride) {
            if (count == 0) {
                return true;
        ...
        if (versionSize == 0 || !string_at(reader, strings, versionOffset, versionSize,
        ...
        for (std::size_t index = 0; index < linkedRefs.size(); ++index) {
            if (!reader.read_u32(record_offset(sections[kLinkedRefSection], index), linkedRefs[index])) {
        ...
        for (std::size_t index = 0; index < sections[kGraphSection].count; ++index) {
            ...
            if (!reader.read_u32(offset, graph.hash)
                || !reader.read_u32(offset + 4U, nodeStart)
                ...
                || linkedStart > linkedRefs.size() || linkedCount > linkedRefs.size() - linkedStart) {
