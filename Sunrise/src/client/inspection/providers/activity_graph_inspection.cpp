...
[[nodiscard]] ActivityMetadata metadata_for(const activity_catalog::Catalog& catalog,
    ...
    metadata.buildMatch =
        activity_catalog::compatibility(catalog) == activity_catalog::Compatibility::compatible;
...
append(Graph& graph, std::vector<Diagnostic>& diagnostics, const Source& source, NodeId parent) {
    AppendResult result{};
    result.present =
        g_state.load.compatibility == activity_catalog::Compatibility::compatible
    ...
    const auto release_count = [&releaseCounts](std::uint32_t graphHash, std::uint32_t nodeHash) {
    ...
    diagnostics.push_back(
        {result.buildMatch ? Diagnostic::Severity::information : Diagnostic::Severity::warning,
         result.buildMatch
             ? "Activity catalog loaded for the target build; authored positions are browse "
               "metadata."
