...
/** Captures no gameplay pointers; the player module has already copied these fields on a game
 * thread. */
...
[[nodiscard]] inline std::string label(const Snapshot& snapshot) {
    ...
    const int written = std::snprintf(
        text.data(), text.size(), "Local controlled object 0x%08X", snapshot.controlledHandle);
