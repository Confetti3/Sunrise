...
struct SlotSnapshot final {
    ...
    [[nodiscard]] friend bool operator==(const SlotSnapshot&, const SlotSnapshot&) noexcept = default;
};
 ...
 */
inline void collect(const layouts::Definition& layout,
                    std::int32_t bubble,
                    std::vector<Snapshot>& output) {
    constexpr std::size_t kGroupIndexCapacity = layouts::kDestinationGroupCapacity
                                                + layouts::kDestinationBubbleGroupCapacity;
    ...
    std::sort(output.begin(),
              output.end(),
              [](const Snapshot& left, const Snapshot& right) noexcept {
...
[[nodiscard]] inline AppendResult append(Graph& graph,
    ...
    const int groupWritten = std::snprintf(groupLabel.data(),
                                           ...
                                           objects.size());
    groupNode.name = groupWritten > 0
                             && static_cast<std::size_t>(groupWritten) < groupLabel.size()
    ...
    if (coverageWritten > 0
        && static_cast<std::size_t>(coverageWritten) < coverage.size()) {
        diagnostics.push_back(
