...
void readiness_row(const char* group,
                   const char* coverage,
                   const world_inspector::ReadinessSummary& summary) noexcept {
    ImGui::TableNextRow();
    ImGui::TableNextColumn();
    ImGui::TextUnformatted(group);
    ImGui::TableNextColumn();
    ImGui::TextWrapped("%s", coverage);
    ImGui::TableNextColumn();
    ImGui::Text("%zu / %zu", summary.readyProducers, summary.producers);
    ImGui::TableNextColumn();
    ImGui::Text("%llu / %llu%s",
                static_cast<unsigned long long>(summary.copied),
                static_cast<unsigned long long>(summary.declared),
                summary.truncated ? " (bounded)" : "");
}

void draw_inspector_entry() noexcept {
    section::header(
        "World Inspector",
        "Browse copied runtime observations and authored evidence without enabling Viewer Camera.");
    const world_inspector::EntrySnapshot snapshot = world_inspector::entry_snapshot();
    const float readyWidth =
        ImGui::CalcTextSize("000 / 000").x + ImGui::GetStyle().CellPadding.x * 2.0F;
    const float copiedWidth =
        ImGui::CalcTextSize("00000 / 00000 (bounded)").x + ImGui::GetStyle().CellPadding.x * 2.0F;
    if (ImGui::BeginTable("##viewer_inspector_readiness",
                          4,
                          ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerH
                              | ImGuiTableFlags_SizingStretchProp)) {
        ImGui::TableSetupColumn("Group", ImGuiTableColumnFlags_WidthFixed);
        ImGui::TableSetupColumn("Coverage", ImGuiTableColumnFlags_WidthStretch, 1.0F);
        ImGui::TableSetupColumn("Ready", ImGuiTableColumnFlags_WidthFixed, readyWidth);
        ImGui::TableSetupColumn("Copied", ImGuiTableColumnFlags_WidthFixed, copiedWidth);
        ImGui::TableHeadersRow();
        readiness_row(
            "Runtime", "Local player, objects, triggers, audio, and physics", snapshot.runtime);
        readiness_row("Authored",
                      "Placed content, spawns, bounds, Activity Map, and relationships",
                      snapshot.authored);
        readiness_row("Rendering", "Depth-tested world helpers", snapshot.rendering);
        ImGui::EndTable();
    }

    ImGui::Spacing();
    if (!snapshot.sessionPresent) {
        ImGui::TextDisabled("No copied activity session is available.");
    } else {
        ImGui::Text("World: %s / %s",
                    snapshot.packageName.empty() ? "unresolved" : snapshot.packageName.c_str(),
                    snapshot.mapStem.empty() ? "unresolved" : snapshot.mapStem.c_str());
        ImGui::TextDisabled("Session 0x%016llX | revision %llu%s",
                            static_cast<unsigned long long>(snapshot.activitySession),
                            static_cast<unsigned long long>(snapshot.valueRevision),
    ...
    ImGui::SameLine();
    ImGui::TextDisabled("World | Node Graph | Relationships | Activity Map");
}
...
[[nodiscard]] camera::PlaybackPath runtime_path(const camera_paths::CameraPath& path,
    ...
    for (std::size_t index = 0; index < result.keyframeCount; ++index) {
        ...
        std::snprintf(
            destination.label.data(), destination.label.size(), "%s", source.label.c_str());
...
void draw_camera_paths(const camera::Status& status, const WorldContext& world) noexcept {
    ...
    if (ImGui::Combo(
            "##camera_path", &selected, names.data(), static_cast<int>(library.paths.size()))) {
    ...
    if (ImGui::Button("Record current pose")) {
        ...
        keyframe.fov = std::clamp(
            status.pose.fov, camera_paths::kMinimumKeyframeFov, camera_paths::kMaximumKeyframeFov);
        ...
        if (world_inspector::selected_identity(selection)) {
            keyframe.selection = camera_paths::SelectionIdentity{
                selection.producerEpoch, selection.nativeKey, selection.producer, selection.kind};
        }
    ...
    if (playback.playing && playback.durationSeconds > 0.0F) {
        ...
        if (ImGui::SliderFloat(
                "Scrub##camera_path", &scrub, 0.0F, playback.durationSeconds, "%.2fs")) {
    ...
    for (std::size_t index = 0; index < path.keyframes.size(); ++index) {
        ...
        ImGui::SameLine();
        persist = ImGui::DragFloat("Travel",
                                   &keyframe.travelSeconds,
                                   0.05F,
                                   0.0F,
                                   camera_paths::kMaximumSegmentSeconds,
                                   "%.2fs")
                  || persist;
        ImGui::SameLine();
        persist = ImGui::DragFloat("Dwell",
                                   &keyframe.dwellSeconds,
                                   0.05F,
                                   0.0F,
                                   camera_paths::kMaximumSegmentSeconds,
                                   "%.2fs")
