...
struct Limits final {
    ...
    std::size_t hierarchyLimit{4096};
};
...
[[nodiscard]] inline bool eligible(
...
[[nodiscard]] inline Result selected_neighborhood(
    ...
    if (root != nullptr && !collapsed.contains(result.root.value)) {
        ...
        if (selectedIterator != siblings.end()) {
            ...
            for (std::size_t index = 0;
                 index < (std::min)(siblings.size(), limits.siblingContext);
                 ++index) {
                admit(siblings[index], eligibleNodes, output);
            }
        }
...
[[nodiscard]] inline Result filtered_hierarchy(
    const inspection::Graph& graph,
