...
relationship_revision(const inspection::ActivityLogicMetadata& metadata) noexcept {
...
Result draw_activity_logic_relationships(const inspection::Graph& graph,
    ...
    if (state.cachedGeneration != graph.generation() || state.cachedAdmissionRevision != revision
        || state.cachedRoot != entity->id) {
        const bool firstLayout = state.layout.empty();
        const std::uint64_t identity = root_identity(graph, entity->id);
        const bool rootChanged = state.cachedRootIdentity != identity;
        state.layout.clear();
        ...
        for (const inspection::ActivityLogicRelationship& relationship : metadata.relationships) {
            ...
            const auto [iterator, inserted] = targetIndex.emplace(id.value, targets.size());
            if (inserted) {
    ...
    if (entityLayout != nullptr) {
        ...
        for (const inspection::ActivityLogicRelationship& relationship : metadata.relationships) {
            ...
            draw_arrowhead(*drawList, to, relationship.outgoing, color, scaled(1.6F));
            if (state.zoom >= 0.68F) {
