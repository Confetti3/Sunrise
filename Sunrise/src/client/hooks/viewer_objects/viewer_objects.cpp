...
[[nodiscard]] std::uint32_t current_handle(const std::array<std::byte, kIteratorBytes>& iterator,
    ...
    const std::uint16_t slot =
        index < limit ? static_cast<std::uint16_t>(index) : kIteratorEnd;
...
bool install() noexcept {
    ...
    g_objectStride = reinterpret_cast<std::uint32_t*>(
        patterns::resolve_relative(finish + 23, finish + 27));
    g_objectBase =
...
void poll() noexcept {
    ...
    for (std::uint32_t visited = 0;; ++visited) {
        ...
        if (next.objectCount < next.objects.size()) {
            ...
            output.type = read<std::uint8_t>(base + static_cast<std::size_t>(slot) * stride
                                             + kObjectType);
