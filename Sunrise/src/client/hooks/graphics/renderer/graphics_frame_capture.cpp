...
std::atomic_uint64_t g_frameSequence{};
...
void release_objects(Storage& storage) noexcept {
    release_com(storage.renderTarget);
...
[[nodiscard]] bool create(ID3D11Device* device,
    ...
    bool renderable = SUCCEEDED(device->CreateTexture2D(&target, nullptr, &storage.texture));
    if (!renderable) {
    ...
    if (FAILED(device->CreateShaderResourceView(storage.texture, &view, &storage.view))) {
    ...
    if (renderable) {
        ...
        renderTarget.Format = viewFormat;
        renderTarget.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
        renderTarget.Texture2D.MipSlice = 0;
        if (FAILED(device->CreateRenderTargetView(
                storage.texture, &renderTarget, &storage.renderTarget))) {
            // The frame capture remains usable as a plain SRV; only the experimental
            // depth-tested helper pass is unavailable for this descriptor.
            storage.renderTarget = nullptr;
        }
    }
    return true;
}
...
bool update(ID3D11Device* device,
    ...
    if (FAILED(
            swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&source)))
        || source == nullptr) {
        release_com(source);
        return false;
    }
    ...
    storage.frameId = g_frameSequence.fetch_add(1, std::memory_order_relaxed) + 1U;
    release_com(source);
    return true;
}
...
ID3D11RenderTargetView* render_target(const Storage& storage) noexcept {
    return storage.renderTarget;
}
