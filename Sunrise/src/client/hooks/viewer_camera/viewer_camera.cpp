...
#include "viewer_camera_position.h"
...
std::atomic<Phase> g_phase{Phase::inactive};
...
std::atomic_uint32_t g_nativePoseSequence{};
Pose g_nativePose{};
std::atomic_bool g_nativePosePresent{};
...
std::atomic<float> g_outputFov{kNativeFov};
std::atomic<float> g_nativeFov{kNativeFov};
...
void publish_native_pose(const Pose& pose) noexcept {
    g_nativePoseSequence.fetch_add(1, std::memory_order_acq_rel);
    g_nativePose = pose;
    g_nativePoseSequence.fetch_add(1, std::memory_order_release);
    g_nativePosePresent.store(true, std::memory_order_release);
}

[[nodiscard]] bool native_pose_snapshot(Pose& pose) noexcept {
    if (!g_nativePosePresent.load(std::memory_order_acquire)) {
        return false;
    }
    for (;;) {
        const std::uint32_t before = g_nativePoseSequence.load(std::memory_order_acquire);
        if ((before & 1U) != 0U) {
            continue;
        }
        pose = g_nativePose;
        if (g_nativePoseSequence.load(std::memory_order_acquire) == before) {
            break;
        }
    }
    return g_nativePosePresent.load(std::memory_order_acquire);
}
...
[[nodiscard]] Pose
playback_pose(const PlaybackPath& path, float elapsed, std::size_t& keyframeIndex) noexcept {
        ...
        if (remaining <= travel) {
            ...
            for (std::size_t lane = 0; lane < result.position.size(); ++lane) {
                result.position[lane] =
                    from.pose.position[lane]
                    + (to.pose.position[lane] - from.pose.position[lane]) * blend;
            }
            const float yawDelta =
                std::remainder(to.pose.yaw - from.pose.yaw, 2.0F * std::numbers::pi_v<float>);
...
void enqueue_capture_range(const PlaybackPath& path,
    ...
    for (std::size_t index = 0; index < path.keyframeCount; ++index) {
        const bool crossed =
            (includeFirst && arrival == 0.0F) || (arrival > after && arrival <= through);
...
float __fastcall copy_fov() noexcept {
    ...
    g_nativeFov.store(native, std::memory_order_release);
    float output = native;
...
bool __fastcall copy_pose(float* position, float* forward, float* up) noexcept {
    ...
    (void)normalize(nativeForward);
    (void)normalize(nativeUp);
    Pose nativePose{};
    nativePose.position = nativePosition;
    nativePose.forward = nativeForward;
    nativePose.up = nativeUp;
    nativePose.yaw = std::atan2(nativeForward[1], nativeForward[0]);
    nativePose.pitch = std::asin(std::clamp(nativeForward[2], -1.0F, 1.0F));
    nativePose.fov = g_nativeFov.load(std::memory_order_acquire);
    if (!std::isfinite(nativePose.fov) || nativePose.fov <= 0.0F) {
        nativePose.fov = kNativeFov;
    }
    publish_native_pose(nativePose);
    ...
    if (!g_state.active) {
        ...
        g_phase.store(Phase::active, std::memory_order_release);
        publish_pose_locked();
...
[[nodiscard]] bool publish_claimed_keys(const client::viewer::Settings& settings) noexcept {
    if (!read_bindings() || !hooks::teleport::action_keys_ready()) {
        // Missing metadata must never turn Viewer into a global keyboard grab.
...
void finish_exit(Failure failure = Failure::none) noexcept {
    ...
    ReleaseSRWLockExclusive(&g_stateLock);
    g_phase.store(failure == Failure::none ? Phase::inactive : Phase::faulted,
...
void update_toggle(const client::viewer::Settings& settings, bool uiVisible) noexcept {
    ...
    if (pressed && !uiVisible) {
        request_active(!g_requested.load(std::memory_order_acquire));
    }
}
...
void update_motion(const client::viewer::Settings& settings, bool uiVisible) noexcept {
    ...
    if (g_state.playbackScrub >= 0.0F && g_state.playbackPlaying) {
        g_state.playbackElapsed =
            std::clamp(g_state.playbackScrub, 0.0F, playback_duration(g_state.playback));
        g_state.playbackScrub = -1.0F;
    ...
    if (g_state.playbackPlaying) {
        ...
        const Pose nextPose =
            playback_pose(g_state.playback, g_state.playbackElapsed, g_state.playbackKeyframe);
        if (!finite(nextPose.position) || !finite(nextPose.forward) || !finite(nextPose.up)
            || !std::isfinite(nextPose.yaw) || !std::isfinite(nextPose.pitch)
            || !std::isfinite(nextPose.fov)) {
            clear_playback_locked();
            g_failure.store(Failure::unsafePosition, std::memory_order_release);
            ReleaseSRWLockExclusive(&g_stateLock);
            report("playback", "fail", "unsafe_position");
            return;
        }
        g_state.pose = nextPose;
        if (!playbackScrubbed) {
            if (wrapped) {
                enqueue_capture_range(g_state.playback, previousElapsed, duration, playbackStarted);
                enqueue_capture_range(g_state.playback, -1.0F, g_state.playbackElapsed, true);
            } else {
                enqueue_capture_range(
                    g_state.playback, previousElapsed, g_state.playbackElapsed, playbackStarted);
            }
        }
    ...
    if ((uiVisible && !workspace_input::workspace_navigation()) || !window_input::game_focused()) {
        ReleaseSRWLockExclusive(&g_stateLock);
        return;
    }
    ...
    if (normalize(movement)) {
        ...
        Vector candidate = g_state.pose.position;
        for (std::size_t lane = 0; lane < movement.size(); ++lane) {
            candidate[lane] += movement[lane] * distance;
        }
        if (finite(candidate)) {
            g_state.pose.position = candidate;
            if (g_failure.load(std::memory_order_acquire) == Failure::unsafePosition) {
                g_failure.store(Failure::none, std::memory_order_release);
            }
        } else {
            g_failure.store(Failure::unsafePosition, std::memory_order_release);
...
bool install() noexcept {
    ...
    g_nativePosePresent.store(false, std::memory_order_release);
...
void poll(std::uint32_t playerIndex) noexcept {
    ...
    if (!requested) {
        if (g_phase.load(std::memory_order_acquire) != Phase::faulted) {
            g_phase.store(Phase::inactive, std::memory_order_release);
        }
        hooks::polled_input::clear_claimed_keys();
        return;
    ...
    AcquireSRWLockShared(&g_stateLock);
    const bool presentationActive = g_state.presentation.active;
    ReleaseSRWLockShared(&g_stateLock);
...
bool captures_mouse() noexcept {
    return g_installed.load(std::memory_order_acquire) && g_active.load(std::memory_order_acquire)
           && g_phase.load(std::memory_order_acquire) == Phase::active
           && !g_stopping.load(std::memory_order_acquire)
           && (!core::ui::runtime::snapshot().visible || workspace_input::workspace_navigation())
...
void request_active(bool active) noexcept {
    ...
    const bool engaged = g_requested.exchange(false, std::memory_order_acq_rel)
                         || g_active.load(std::memory_order_acquire)
                         || g_activeSession.load(std::memory_order_acquire) != 0;
    g_phase.store(engaged ? Phase::exiting : Phase::inactive, std::memory_order_release);
}
...
bool move_to(const Pose& pose) noexcept {
    if (!finite(pose.position) || !std::isfinite(pose.yaw) || !std::isfinite(pose.pitch)
        ...
        g_failure.store(Failure::unsafePosition, std::memory_order_release);
        report("move", "fail", "unsafe_position");
        return false;
    }
    AcquireSRWLockExclusive(&g_stateLock);
    if (!g_state.active) {
...
Status status() noexcept {
    ...
    if (g_state.active) {
        ...
        snapshot.pose.fov = g_playbackFovActive.load(std::memory_order_acquire)
                                ? g_playbackFov.load(std::memory_order_acquire)
                                : g_outputFov.load(std::memory_order_acquire);
        snapshot.poseSource = PoseSource::viewer;
        snapshot.projectionAvailable = true;
    }
    ReleaseSRWLockShared(&g_stateLock);
    if (!snapshot.active) {
        Pose native{};
        if (native_pose_snapshot(native)) {
            snapshot.pose = native;
            snapshot.poseSource = PoseSource::player;
            snapshot.projectionAvailable = true;
        }
    }
    snapshot.failure = g_failure.load(std::memory_order_acquire);
    snapshot.phase = g_phase.load(std::memory_order_acquire);
...
const char* phase_name(Phase phase) noexcept {
    ...
    case Phase::inactive:
        return "PLAYER VIEW";
    case Phase::entering:
        return "ENTERING VIEWER";
    case Phase::active:
        return "VIEWER ACTIVE";
    case Phase::exiting:
        return "EXITING VIEWER";
    case Phase::faulted:
        return "VIEWER FAULTED";
    }
    return "VIEWER UNKNOWN";
}

const char* pose_source_name(PoseSource source) noexcept {
    switch (source) {
    case PoseSource::unavailable:
        return "NO CAMERA POSE";
    case PoseSource::player:
        return "PLAYER CAMERA";
    case PoseSource::viewer:
...
const char* failure_name(Failure failure) noexcept {
    ...
    case Failure::unsafePosition:
        return "camera position is not finite";
