...
void publish_volume(void* postSimulationListener) noexcept {
    ...
    next.positionPresent =
        std::ranges::all_of(next.position, [](float lane) noexcept { return std::isfinite(lane); });
    if (!next.positionPresent) {
        return;
    }
    const std::int32_t overlapCount = read<std::int32_t>(volume + kTriggerVolumeOverlapCountOffset);
...
bool install() noexcept {
    ...
    if (eventTarget == nullptr || volumeTarget == nullptr) {
        ...
        core::log::write(core::log::Channel::client,
                         core::log::Level::warn,
                         "ev=viewer_triggers stage=install result=fail reason=signature");
        return false;
    }
    if (!hooking::detour::install(specs, g_handles)) {
        ...
        core::log::write(core::log::Channel::client,
                         core::log::Level::warn,
                         "ev=viewer_triggers stage=install result=fail reason=detour_attach");
        return false;
    }
...
bool uninstall() noexcept {
    ...
    if (!replacement_idle()) {
        core::log::write(
            ...
            core::log::Level::warn,
            "ev=viewer_triggers stage=uninstall result=fail reason=replacement_active");
        return false;
    }
    if (g_handles[0].attached || g_handles[1].attached) {
        ...
        const hooking::detour::UninstallResult removal =
            hooking::detour::uninstall(g_handles, protectedEntries, &replacement_idle);
        if (removal != hooking::detour::UninstallResult::removed) {
            core::log::write(core::log::Channel::client,
                             core::log::Level::warn,
                             "ev=viewer_triggers stage=uninstall result=fail reason=detour_detach");
            return false;
        }
    }
    g_handles = {};
    ...
    core::log::write(core::log::Channel::client,
                     core::log::Level::info,
                     "ev=viewer_triggers stage=uninstall result=ok");
    return true;
}
